/* Research Office Controller — Zen Flow + Drafts Manager (safe + search)
   ...
   - Caret restoration
   - Drafts Manager: show all / current, search filter, open here, open, delete one
...
(() => {
  ...
  const LS_KEY = 'ro:memos'; // array of { ts, chapter, paraId, body }
  ...
  function ensureDraftsControls(){
    ...
    wrap.id = 'draftsControls';
    wrap.className = 'bar';

    // Filter toggle buttons
    const btnShowAll = document.createElement('button');
    btnShowAll.className = 'btn btn-sm';
    btnShowAll.id = 'btnShowAllDrafts';
    btnShowAll.textContent = 'Show all drafts';

    const btnShowCurrent = document.createElement('button');
    btnShowCurrent.className = 'btn btn-sm';
    btnShowCurrent.id = 'btnShowCurrentDrafts';
    btnShowCurrent.textContent = 'Show only this paragraph';

    // Search input
    const search = document.createElement('input');
    search.id = 'draftSearch';
    search.type = 'search';
    ...
    countBadge.className = 'tag draft-count';
  ...
  // Keep/refresh an entry for this chapter+para on every memo change
    ...
    if (!list.length){
      ...
      list.forEach((d) => {
        ...
        info.innerHTML = `
          ...
          <div class="mono" style="opacity:.7;margin-top:4px">
            chapter: ${escapeHtml(d.chapter)} • para: ${escapeHtml(d.paraId)}
          </div>
        `;
        ...
        // Open here (only when it belongs to this paragraph)
        const btnHere = document.createElement('button');
        btnHere.className = 'btn btn-sm';
        btnHere.textContent = 'Open here';
        ...
        btnHere.addEventListener('click', () => {
          ...
          memoTa.value = d.body || '';
          onMemoChange();
          const end = memoTa.value.length;
          memoTa.setSelectionRange(end, end);
          memoTa.focus({ preventScroll: true });
          const s = document.getElementById('memoStatus');
          if (s) s.innerHTML += ` <span class="tag">Opened</span>`;
        });

        // Open (navigate) — for any draft
        const btnOpen = document.createElement('button');
        btnOpen.className = 'btn btn-sm';
        btnOpen.textContent = 'Open';
        btnOpen.title = 'Navigate to this draft’s paragraph';
        ...
        // Delete this draft (single-item delete only)
        const btnDel = document.createElement('button');
        btnDel.className = 'btn btn-sm';
        btnDel.textContent = 'Delete';
        btnDel.title = 'Remove this draft from browser storage';
        btnDel.addEventListener('click', () => {
          ...
          if (idx >= 0) {
            all.splice(idx, 1);
            saveAllDrafts(all);
            renderDraftList(all, filter, query);
          }
        });
