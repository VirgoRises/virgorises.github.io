...
// Full HTML for a separate window (includes MathJax)
function buildHTMLWindow(baseHref) {
  const CSS = `
...
<body>
  ...
  <script>
    ...
    async function typesetReady(){
      for(let i=0;i<60;i++){ if(window.MathJax?.startup?.promise) break; await new Promise(r=>setTimeout(r,100)); }
    ...
    async function typeset(c){
      ...
      await typesetReady();
      try{
        MathJax.typesetClear?.([c]); MathJax.texReset?.();
        if(MathJax.typesetPromise){ await MathJax.typesetPromise([c]); }
        else { MathJax.typeset?.([c]); }
      }catch(_){}
    }
    ...
    function renderList(name){
      ...
      if(name==='current'){
        ...
        const btn=document.createElement('button'); btn.className='btn btn-sm'; btn.textContent='Use this version';
        btn.style='margin-top:6px'; btn.addEventListener('click',()=>parent.postMessage({kind:'ro_set_memo',body:PAYLOAD.currentBodyPlain},'*'));
        d.appendChild(btn); lists.appendChild(d);
        container.innerHTML=PAYLOAD.html; typeset(container); stamp.textContent=new Date(PAYLOAD.ts).toLocaleTimeString();
      }

      if(name==='autosaves'){
        if(!PAYLOAD.history?.length){ lists.innerHTML='<div class="item mono">No autosaves.</div>'; }
        else{
          PAYLOAD.history.forEach((h,i)=>{
            ...
            row.innerHTML=\`
              ...
              <div style="display:flex;gap:6px;margin-top:8px">
                <button class="btn btn-sm" data-act="preview" data-i="\${i}">Preview</button>
                <button class="btn btn-sm" data-act="use" data-i="\${i}">Use this version</button>
              </div>\`;
      ...
      if(name==='drafts'){
        if(!PAYLOAD.drafts?.length){ lists.innerHTML='<div class="item mono">No drafts stored in this browser.</div>'; }
        else{
          PAYLOAD.drafts.forEach((d,i)=>{
            ...
            row.innerHTML=\`
              ...
              <div style="display:flex;gap:6px;margin-top:8px">
                <button class="btn btn-sm" data-act="preview" data-i="\${i}">Preview</button>
                <button class="btn btn-sm" data-act="use" data-i="\${i}" \${d.isCurrent?'':'disabled'}>Use here</button>
                <button class="btn btn-sm" data-act="open" data-i="\${i}">Open draft</button>
              </div>\`;
            lists.appendChild(row);
          });
          lists.addEventListener('click',async ev=>{
...
// Markup for same-document dialog (NO MathJax scripts)
function buildHTMLDialog() {
  return `
  <header style="padding:10px 14px;border-bottom:1px solid #1f2a36;background:#0F141A">
    <strong>Live Preview</strong><span class="mono" id="stamp" style="float:right;opacity:.8"></span>
  </header>
  <div id="wrap" style="display:grid;grid-template-columns:280px 1fr;min-height:calc(90vh - 40px);min-width:0">
    <aside id="side" style="border-right:1px solid #1f2a36;background:#0F141A">
      <div id="tabs" style="display:flex;gap:6px;padding:8px;border-bottom:1px solid #1f2a36">
        <button id="tab-current" class="active">Current</button>
        <button id="tab-autosaves">Autosaves</button>
        <button id="tab-drafts">Drafts</button>
      </div>
      <div id="lists" style="overflow:auto;padding:8px"></div>
    </aside>
    <main id="container" style="padding:16px;max-width:980px;margin:0 auto"></main>
  </div>`;
}
...
function buildPayload(){
  const hist = (getHistory()||[]).slice(0,20).map(s=>({
    ts:s.ts, body:s.body,
    html: window.marked?.parse ? window.marked.parse(s.body) :
         `<pre>${(s.body||'').replace(/[&<>]/g,m=>({ '&':'&amp;','<':'&lt;','>':'&gt;' }[m]))}</pre>`
  }));
  ...
  const drafts = draftsRaw.map(d=>({
    ...
    body:d.body,
    html: window.marked?.parse ? window.marked.parse(d.body||'') :
         `<pre>${(d.body||'').replace(/[&<>]/g,m=>({ '&':'&amp;','<':'&lt;','>':'&gt;' }[m]))}</pre>`
  }));
  return {
    kind:'ro_preview',
    ts: Date.now(),
...
function openPreview(ev){
  ...
  if (!w) return openDialog();

  if (w.document && !w.document.body.childElementCount) {
    w.document.open(); w.document.write(buildHTMLWindow(baseHref)); w.document.close();
  }
  previewWin = w; push();
}
...
function openDialog(){
  if (!previewDlg){
    ...
    previewDlg.id='roLivePreviewDlg';
    previewDlg.style.cssText='width:min(1100px,calc(100vw - 48px));max-height:90vh;padding:0;border:none;border-radius:12px;overflow:hidden;background:#0B1117;color:#E6EDF3;';
    previewDlg.innerHTML = buildHTMLDialog();   // IMPORTANT: no MathJax injected here
    document.body.appendChild(previewDlg);
  }
  if (!previewDlg.open) previewDlg.showModal();
  push(); // uses parent’s MathJax to typeset
}

// Send current preview to window & dialog
function push(){
  ...
  // dialog (same document): update nodes and typeset with parent MathJax
...
// Keep pop-out in sync whenever memo preview updates
