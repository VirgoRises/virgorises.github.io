...
//   pdfUrl: "/data/.../Old_main.pdf",
//   initialZoom: "fit",         // "fit" | number (e.g. 1.2)
// })
//
// If you still want static images instead of PDF, pass:
//   pageToUrl: (p)=>".../page-<p>.jpg"
// (pageToUrl wins over pdfUrl)
...
export function initResearchOfficeGrid(opts = {}) {
  ...
  const img    = document.getElementById("ro-page");      // kept for image fallback
  if (!stage || !img) return;
  ...
  // Toolbar (extend existing if present)
  ...
  let pdf = null;
  let pdfPage = null;
  let pageViewport = null;      // unscaled PDF viewport @ scale=1
  let zoomMode = initialZoom;   // "fit" or number
  let isImageMode = false;      // true if using pageToUrl image
  ...
  (async () => {
    ...
    if (imgUrl) {
      ...
      img.onload = () => {
        // Match overlay to natural image pixels
        setSvgViewBox(img.naturalWidth, img.naturalHeight);
      };
    ...
    if (pdfUrl && fromPage > 0) {
      ...
      pageViewport = pdfPage.getViewport({ scale: 1 }); // base size
    ...
    img.style.display = "none";
    canvas.style.display = "none";
    setSvgViewBox(1000, 1414); // arbitrary A4 portrait; keeps grid working
    afterBackdropReady();
  })();

  function afterBackdropReady(){
  ...
  async function renderPdfAtCurrentZoom() {
    ...
    // Compute target CSS width (fit-to-stage or zoom factor)
    const stageW = stage.clientWidth || 800;
    let cssWidth;
    if (zoomMode === "fit") {
      cssWidth = stageW;
    } else if (typeof zoomMode === "number" && zoomMode > 0) {
      cssWidth = pageViewport.width * zoomMode; // css pixels
    } else {
      cssWidth = stageW;
    }
  ...
  function observeResize() {
    ...
    const onResize = () => {
      ...
      raf = requestAnimationFrame(async () => {
        ...
        // image mode: overlay viewBox sticks to natural size; CSS width adapts automatically
      });
    };
  ...
  function setSvgViewBox(w, h){
    ...
    // force the SVG to track the visible element’s CSS box
    svg.style.width = "100%";
  ...
  function drawGrid(){
    ...
    for (let r=1;r<grid.rows;r++){
      const y = (r/grid.rows);
      g.appendChild(mk("line",{ x1:0, y1: y*1000, x2:1000, y2: y*1000 }));
    }
    for (let c=1;c<grid.cols;c++){
      const x = (c/grid.cols);
      g.appendChild(mk("line",{ x1: x*1000, y1:0, x2: x*1000, y2:1000 }));
    }
    for (let c=0;c<grid.cols;c++){
      const x = ((c+0.5)/grid.cols)*1000;
      const t = mk("text",{ x, y:16, class:"label", "text-anchor":"middle" });
      t.textContent = String(c+1); g.appendChild(t);
    }
    for (let r=0;r<grid.rows;r++){
      const y = ((r+0.5)/grid.rows)*1000;
      const t = mk("text",{ x:14, y, class:"label", "text-anchor":"middle" });
      t.textContent = rowLabel(r); g.appendChild(t);
    }
    svg.appendChild(g);
  }
  ...
  function snapXY(x,y){
    const cx = (Math.floor(x * grid.cols) + 0.5) / grid.cols;
    const cy = (Math.floor(y * grid.rows) + 0.5) / grid.rows;
    return { x: cx, y: cy };
  }
  ...
  function setToken(s){
    const i = byId("ro-token");
    if (!i) return;
    i.value = s; i.select(); document.execCommand("copy");
    toast("Copied: " + s);
  }
  function toast(msg){
    const t=document.createElement("div"); t.className="ro-toast"; t.textContent=msg;
