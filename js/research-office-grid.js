...
export function initResearchOfficeGrid(opts = {}) {
  ...
  // jh
  // --- init flags to avoid double-binding / double-crosshair
  let __uiBound = false;
  let __crosshairReady = false;
  // jh
  ...
  function bindUIOnce() {
    ...
    const btnClear = document.getElementById("ro-clear");
    const btnExport = document.getElementById("ro-export");
    const zoomOut = document.getElementById("ro-zoom-out");
    const zoomIn = document.getElementById("ro-zoom-in");
    const zoomFit = document.getElementById("ro-zoom-fit");
    ...
    if (btnExport) btnExport.onclick = () => {
      const payload = { docId: "Old_main", chapter, anchor: para, markers, units: (document.getElementById("ro-units")?.value || "mm") };
    ...
    svg.addEventListener("mousedown", (e) => {
      const p = forceSnapMetric(percentPos(e));
      if (mode === "point") {
        markers.push({ kind: "point", x: p.x, y: p.y });
        saveLocal(storeKey, markers);
        renderMarkers();
        commitToken(metricToken(p.x, p.y));  // keep memo integration
      } else {
  ...
  function ensureCrosshair() {
    if (__crosshairReady) return;
    // Avoid duplicates: look for any existing .ro-cross
    if (!svg.querySelector(".ro-cross")) {
      const cx = document.createElementNS("http://www.w3.org/2000/svg", "line");
      cx.setAttribute("class", "ro-cross");
      cx.setAttribute("x1", "0"); cx.setAttribute("y1", "0");
      cx.setAttribute("x2", String(BACK.pxW)); cx.setAttribute("y2", "0");

      const cy = document.createElementNS("http://www.w3.org/2000/svg", "line");
      cy.setAttribute("class", "ro-cross");
      cy.setAttribute("x1", "0"); cy.setAttribute("y1", "0");
      cy.setAttribute("x2", "0"); cy.setAttribute("y2", String(BACK.pxH));

      svg.appendChild(cx); svg.appendChild(cy);

      svg.style.cursor = "crosshair";
      svg.addEventListener("mouseleave", () => { cx.setAttribute("opacity", "0"); cy.setAttribute("opacity", "0"); });
      svg.addEventListener("mouseenter", () => { cx.setAttribute("opacity", "1"); cy.setAttribute("opacity", "1"); });
      svg.addEventListener("mousemove", (e) => {
        const p = percentPos(e);
        const x = p.x * BACK.pxW, y = p.y * BACK.pxH;
        cx.setAttribute("y1", y); cx.setAttribute("y2", y);
        cy.setAttribute("x1", x); cy.setAttribute("x2", x);
      });
    }
    __crosshairReady = true;
  }
  ...
  // JH
  // after the very first successful paint:
  ...
  function redrawAll() {
    ...
    renderMarkers();
    //ensureCrosshair();
    __crosshairReady = false; // force rebuild
    ensureCrosshair();
  }
