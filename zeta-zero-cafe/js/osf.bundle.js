// osf.bundle.js — stable anchors + copy + smooth scroll + tiny “propose” popover
(function () {
  "use strict";
  ...
  const DEFAULTS = {
    SCROLL_OFFSET: 80,
    HILITE_MS: 6000,
    // where your anchors JSON lives (auto-detected from /notebook/ path)
    basePath: null,
    // marketing / join page for non-members:
    squareUrl: "/zeta-zero-cafe/join-the-square.html",
    // the Discord channel for members (set your real URL):
  ...
  function ensureStyles() {
    ...
    const css = `
      ...
      .osf-copy, .osf-more { border:0; background:transparent; font:inherit; line-height:1; cursor:pointer; opacity:.0; padding:0 .15rem; }
      .osf-head:hover .osf-copy, .osf-head:hover .osf-more,
      .osf-copy:focus-visible, .osf-more:focus-visible { opacity:1; }
      ...
      .osf-toast { position:fixed; bottom:1rem; left:50%; transform:translateX(-50%);
                   background:#000; color:#fff; padding:.4rem .6rem; border-radius:.5rem; font-size:.85rem;
                   opacity:0; pointer-events:none; transition:opacity .2s; }
      .osf-toast.show { opacity:.9; }
      /* popover */
      .osf-pop { position:absolute; top:1.6rem; left:0; min-width:220px; max-width:280px;
                 background:var(--card, #111); color:var(--fg,#eee); border:1px solid rgba(255,255,255,.12);
                 border-radius:.6rem; box-shadow:0 6px 24px rgba(0,0,0,.3); padding:.6rem; z-index:50; display:none; }
      .osf-pop.open { display:block; }
      .osf-pop h5 { margin:.1rem 0 .4rem 0; font-size:.9rem; font-weight:600; }
      .osf-btn { display:inline-flex; align-items:center; gap:.35rem; padding:.35rem .55rem; border-radius:.45rem;
                 border:1px solid rgba(255,255,255,.18); background:transparent; color:inherit; cursor:pointer; font-size:.85rem; }
  ...
  function detectBasePath() {
    ...
    const p = location.pathname; const i = p.toLowerCase().indexOf("/notebook/");
  ...
  async function fetchAnchors(base, chapterNo) {
    ...
    try { const r = await fetch(url, {cache:"no-store"}); if (!r.ok) throw new Error(r.status);
      return await r.json();
    } catch (e) { log("anchors JSON not available:", e.message); return null; }
  }
  ...
  async function init() {
    ...
    // Optional URL toggles: ?member=1 or ?member=0
    ...
    pres.forEach((pre, i) => {
      ...
      legacy.id = `osf-${i+1}`; legacy.style.position="relative"; legacy.style.top="-1px";
      ...
      copy.className = "osf-copy"; copy.type="button"; copy.title="Copy link"; copy.setAttribute("aria-label",`Copy link to ${labelText}`);
      copy.textContent = "🔗";

      const more = document.createElement("button");
      more.className = "osf-more"; more.type="button"; more.title="History / Propose edit";
      ...
      copy.addEventListener("click", async (ev) => {
        ...
        try { await navigator.clipboard?.writeText(fullLinkTo(uuid)); } catch {}
        toast("Link copied");
      });
      ...
      more.addEventListener("click", (ev) => {
        ...
        if (open) positionPopover(pop, more); else pop.blur();
      });
  ...
  function buildPopover(labelText, uuid){
    ...
    const title = document.createElement("h5"); title.textContent = `Discuss ${labelText}`;
    ...
    copyBtn.className = "osf-btn"; copyBtn.textContent = "Copy paragraph link";
    copyBtn.addEventListener("click", async () => { try { await navigator.clipboard?.writeText(fullLinkTo(uuid)); toast("Link copied"); } catch {} });

    const toggle = document.createElement("button");
    toggle.className = "osf-btn"; toggle.textContent = isMember()? "I’m not a member" : "I’m a member";
    toggle.addEventListener("click", () => { setMember(!isMember()); updatePrimary(primary); toggle.textContent = isMember()? "I’m not a member" : "I’m a member"; });
    ...
    note.textContent = isMember()? "You’re marked as a member on this device." : "Members go straight to Discord; everyone else enters The Square.";

    // update note whenever we toggle
    const _updateNote = () => { note.textContent = isMember()? "You’re marked as a member on this device." : "Members go straight to Discord; everyone else enters The Square."; };
    toggle.addEventListener("click", _updateNote);
  ...
  function positionPopover(pop, anchorBtn){
    // basic placement: left aligned, below the head row
    const r = anchorBtn.getBoundingClientRect();
    pop.style.left = "0";
    pop.style.top  = "1.6rem";
  }
